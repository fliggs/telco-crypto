...
export type ExpoGoogleAuthenticationAuthorizeOptions = {
  scopes: string[]; // The list of scopes we're requesting access to
};

export type ExpoGoogleAuthenticationAuthorizeResponse = {
  accessToken?: string; // The access token to use for other Google APIs
  grantedScopes?: string[]; // The scopes that the user granted us access to
};
