...
class ExpoGoogleAuthenticationModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("authorize", this@ExpoGoogleAuthenticationModule::authorize)
  }
  ...
  private fun authorize(options: AuthorizeOptions, promise: Promise) {
    val REQUEST_AUTHORIZE = 1
    val requestedScopes = options.scopes.map { s -> Scope(s) }
    val authorizationRequest =
        AuthorizationRequest.builder().setRequestedScopes(requestedScopes).build()

    Identity.getAuthorizationClient(appContext.activityProvider?.currentActivity!!)
        .authorize(authorizationRequest)
        .addOnSuccessListener { authorizationResult: AuthorizationResult ->
            if (authorizationResult.hasResolution()) {
                Log.i(TAG, "Result has resolution")
                // Access needs to be granted by the user
                val pendingIntent = authorizationResult.pendingIntent
                try {
                    appContext.activityProvider?.currentActivity!!.startIntentSenderForResult(
                        pendingIntent!!.intentSender,
                        REQUEST_AUTHORIZE, null, 0, 0, 0, null
                    )
                } catch (e: SendIntentException) {
                    Log.e(TAG, "Couldn't start Authorization UI: " + e.localizedMessage)
                    handleLoginFailure(promise, "ERR_FAIL", Exception("Couldn't start Authorization UI: " + e.localizedMessage))
                }
            } else {
                // Access already granted, continue with user action
                Log.i(TAG, "Directly got access token")
                val authResult = mapOf(
                  "accessToken" to authorizationResult.accessToken,
                  "grantedScopes" to authorizationResult.grantedScopes,
                )
        ...
        .addOnFailureListener { e: Exception? ->
            ...
            handleLoginFailure(promise, "ERR_FAIL", Exception("Failed to authorize"))
        }
  }
